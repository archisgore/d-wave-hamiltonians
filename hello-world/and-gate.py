"""AND gate as a reusable annealing building block.

Two constructions of the same gate (out = a AND b):
  1. from primitives  - the penalty QUBO/BQM built by hand
  2. from dimod        - dimod.generators.and_gate does the algebra for you
...
Run:  source ../ocean/bin/activate && python and-gate.py
"""
...
#
# The AND penalty, derivable by hand, is
#     E = a*b - 2*a*out - 2*b*out + 3*out
# which is 0 exactly on the valid rows {000, 010, 100, 111} and > 0 otherwise:
#   (out,out)=3   -> linear bias: turning out on costs +3...
#   (a,out)=(b,out)=-2  -> ...refunded 2 for each input that is also on...
#   (a,b)=1       -> ...and both-inputs-on without out costs +1.
def and_gate_from_primitives(a: Variable, b: Variable, out: Variable) -> BinaryQuadraticModel:
    linear = {out: 3.0}
    quadratic = {(a, b): 1.0, (a, out): -2.0, (b, out): -2.0}
    return dimod.BinaryQuadraticModel(linear, quadratic, 0.0, dimod.BINARY)


# --- 2. AND gate from the dimod library ------------------------------------
#
...
def and_gate_from_dimod(a: Variable, b: Variable, out: Variable) -> BinaryQuadraticModel:
    return dimod.generators.and_gate(a, b, out)
...
def usage_fix_inputs() -> None:
    ...
    bqm = and_gate_from_dimod("a", "b", "out")
...
def composition_three_input_and() -> None:
    # Wire two gates by SHARING a variable name: 't' is the output of the first
    # gate and an input of the second, so  w = (a AND b) AND c.
    # Adding BQMs merges terms on shared variables; ground states satisfy both.
    bqm = and_gate_from_primitives("a", "b", "t")
    bqm += and_gate_from_primitives("t", "c", "w")
    print("\n== composition: w = a AND b AND c (two chained AND gates) ==")
    print(dimod.ExactSolver().sample(bqm).lowest())


if __name__ == "__main__":
    primitive = and_gate_from_primitives("a", "b", "out")
    library = and_gate_from_dimod("a", "b", "out")
    dump(primitive, "and_gate_from_primitives('a','b','out')")
    print()
    dump(library, "and_gate_from_dimod('a','b','out')")

    truth_table(primitive, "from primitives")
    usage_fix_inputs()
    composition_three_input_and()
